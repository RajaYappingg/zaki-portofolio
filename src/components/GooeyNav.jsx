...
const GooeyNav = ({
  ...
  useEffect(() => {
    ...
    const activeLi = navRef.current.querySelectorAll('.gooey-nav-item')[activeIndex];
    if (activeLi) {
      updateEffectPosition(activeLi);
      textRef.current?.classList.add('gooey-active');
    }
    const resizeObserver = new ResizeObserver(() => {
      const currentActiveLi = navRef.current?.querySelectorAll('.gooey-nav-item')[activeIndex];
      if (currentActiveLi) {
    ...
    <>
      <style>
        {`
          ...
          .gooey-effect.text.gooey-active {
            color: black;
          }
          .gooey-effect.filter {
          ...
          .gooey-nav-item.gooey-active {
            color: black;
            text-shadow: none;
          }
          .gooey-nav-item.gooey-active::after {
            opacity: 1;
            transform: scale(1);
          }
        ...
        <nav
          ...
          <ul
            ...
            {items.map((item, index) => (
              <li
                key={index}
                className={`gooey-nav-item rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] text-white ${activeIndex === index ? 'gooey-active' : ''
                  }`}>
                <Link
                  to={item.href}
